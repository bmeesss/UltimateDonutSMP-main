    ...
    private void buildEmptyState() {
        ...
        set(menus().getInt(path + ".SLOT", 22), ItemUtils.createItem(
                ...
                replaceStaticPlaceholders(defaultIfEmpty(
                        ...
                        java.util.Collections.singletonList("&7Requests sent while this setting was disabled will appear here.")
                ))
        ));
    }

    private void buildNavigation(int totalItems, int totalPages) {
        buildButton("PREVIOUS", hasPreviousPage, new java.util.LinkedHashMap(){{ put(
                "page",  String.valueOf(page + 1)); put(
                "previous_page",  String.valueOf(Math.max(1, page))); put(
                "next_page",  String.valueOf(Math.min(totalPages, page + 2))); put(
                "total_pages",  String.valueOf(totalPages)); put(
                "total",  String.valueOf(totalItems)
        ); }});

        buildButton("RANDOM", totalItems > 0, new java.util.LinkedHashMap(){{ put(
                "page",  String.valueOf(page + 1)); put(
                "previous_page",  String.valueOf(Math.max(1, page))); put(
                "next_page",  String.valueOf(Math.min(totalPages, page + 2))); put(
                "total_pages",  String.valueOf(totalPages)); put(
        ...
        buildButton("NEXT", hasNextPage, new java.util.LinkedHashMap(){{ put(
                "page",  String.valueOf(page + 1)); put(
                "previous_page",  String.valueOf(Math.max(1, page))); put(
                "next_page",  String.valueOf(Math.min(totalPages, page + 2))); put(
                "total_pages",  String.valueOf(totalPages)); put(
