...
public class ProfileViewerHomesMenu extends BaseMenu {
    ...
    private void buildEmptyState() {
        ...
        if (lore.isEmpty()) {
            lore = java.util.Collections.singletonList("&7This player has no homes saved.");
        }
    ...
    private ItemStack buildHomeItem(Home home) {
        ...
        if (lore.isEmpty()) {
            lore = valid
                    ? new java.util.ArrayList<>(java.util.Arrays.asList("&7World: &f{world}",  "&7X: &f{x} &7y: &f{y} &7z: &f{z}",  "&aClick to teleport"))
                    : java.util.Collections.singletonList("&7This home points to an unavailable world.");
        }
    ...
    private void buildBackButton() {
        ...
        List<String> fallbackLore = returnToProfileViewer
                ? java.util.Collections.singletonList("&7Return to the main profile viewer.")
    ...
    private void buildRefreshButton() {
        set(REFRESH_SLOT, ItemUtils.createItem(
                ...
                replaceSnapshotPlaceholders(defaultIfEmpty(
                        ...
                        java.util.Collections.singletonList("&7Reload this player's homes.")
                ))
        ));
    }
    ...
    private void buildPageButtons(int totalHomes) {
        ...
        set(PAGE_INFO_SLOT, ItemUtils.createItem(
                ...
                java.util.Collections.singletonList("&fHomes: &7" + NumberUtils.format(totalHomes))
        ));
