...
public class TeamMenu extends BaseMenu {
    ...
    public void build(Player player) {
        ...
        if (members.isEmpty()) {
            set(22, ItemUtils.createItem(
                    ...
                    "&cNo members found",
                    searchQuery == null
                            ? java.util.Collections.singletonList("&7Your team has no members to display.")
                            : new java.util.ArrayList<>(java.util.Arrays.asList("&7No team member matched &f" + searchQuery,  "&7Use search again or right-click search to clear."))
            ));
        }
    ...
    private void renderPageButtons(int totalPages) {
        ...
        set(50, ItemUtils.createItem(
                ...
                java.util.Collections.singletonList("&7Browse team members.")
        ));
    ...
    private void renderPvpButton(Team team) {
        ...
        set(
                ...
                ItemUtils.createItem(
                        ...
                        java.util.Collections.singletonList(replace(menus().getString(path + ".LORE", "&fCurrently: {state}"), new java.util.LinkedHashMap(){{ put("state",  state); }}))
                )
        );
    }
