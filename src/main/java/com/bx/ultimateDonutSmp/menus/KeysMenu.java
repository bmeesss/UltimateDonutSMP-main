...
public class KeysMenu extends BaseMenu {
    ...
    private static final List<Integer> CENTERED_SLOTS = new java.util.ArrayList<>(java.util.Arrays.asList(
    ...
    public void build(Player player) {
        ...
        if (crates.isEmpty()) {
            set(13, ItemUtils.createItem(
                    Material.BARRIER,
                    "&cNo crates",
                    java.util.Collections.singletonList("&7No crate keys are available right now.")
            ));
            return;
        }
    ...
    private ItemStack createKeyItem(Player player, CrateManager.CrateDefinition crate) {
        ...
        ItemStack item = ItemUtils.createItem(
                ...
                java.util.Collections.singletonList("&7Keys: &f" + keyCount)
        );
