...
public class FeatureToggleMenu extends BaseMenu {
    ...
    private org.bukkit.inventory.ItemStack createFeatureItem(FeatureManager.Feature feature) {
        ...
        if (lore.isEmpty()) {
            lore = new java.util.ArrayList<>(java.util.Arrays.asList("&7{description}",  "",  "&7Status: {state}",  "&eClick to toggle."));
        }
    ...
    private void renderButton(String key, boolean active, int safePage, int totalPages) {
        ...
        Map<String, String> placeholders = new java.util.LinkedHashMap(){{ put(
                "page",  String.valueOf(safePage + 1)); put(
                "next_page",  String.valueOf(Math.min(totalPages, safePage + 2))); put(
                "previous_page",  String.valueOf(Math.max(1, safePage))); put(
