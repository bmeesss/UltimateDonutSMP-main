...
public class HideListMenu extends BaseMenu {
    ...
    public void build(Player player) {
        ...
        List<HideState> states = plugin.getHideManager().getStates().stream().collect(java.util.stream.Collectors.toList());
    ...
    private void renderNavigation(int total) {
        ...
        set(49, ItemUtils.createItem(Material.BARRIER, "&cBack", java.util.Collections.emptyList()));
        if ((page + 1) * PAGE_SIZE < total) {
            set(53, ItemUtils.createItem(Material.ARROW, "&bNext page", java.util.Collections.emptyList()));
        }
    }
