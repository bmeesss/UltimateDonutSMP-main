...
public class SellMenu extends BaseMenu {
    ...
    private static final List<SellCategory> BUTTON_ORDER = new java.util.ArrayList<>(java.util.Arrays.asList(
            SellCategory.CROPS, 
            SellCategory.ORES, 
            SellCategory.MOBS, 
    ...
    private void buildConfirmSellGUI(Player player) {
        ...
        if (lore.isEmpty()) {
            lore = java.util.Collections.singletonList("&7Click to sell all items in the menu.");
        }
    ...
    public void handleInventoryDrag(InventoryDragEvent event) {
        ...
        event.setCancelled(false);
        if (isAutoSellEnabled() && event.getWhoClicked() instanceof Player) {
            Player player = (Player) confirmMode) {
            buildConfirmSellGUI(player);
        } else {
            refreshMultiplierButtons(player);
        }
    ...
    public void handleInventoryDrag(InventoryDragEvent event) {
        ...
        event.setCancelled(false);
        if (isAutoSellEnabled() && event.getWhoClicked();
            scheduleProcessing(player);
        }
    }
