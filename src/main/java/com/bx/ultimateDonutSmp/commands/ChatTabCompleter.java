...
public class ChatTabCompleter implements TabCompleter {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            return partialMatches(args[0], new java.util.ArrayList<>(java.util.Arrays.asList("help",  "mute",  "unmute",  "delay",  "clear")));
        }

        if (args.length == 2 && args[0].equalsIgnoreCase("delay")) {
            return partialMatches(args[1], new java.util.ArrayList<>(java.util.Arrays.asList("0",  "3",  "5",  "10",  "off")));
        }
