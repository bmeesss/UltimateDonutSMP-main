...
public class HideCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        return switch (args[0].toLowerCase(Locale.ROOT)) {        case "status": case "check": handleStatus(sender, args, manager)            break;        case "scramble": handleScramble(sender, manager)            break;        case "remove": case "removal": handleRemove(sender, args, manager)            break;        case "list": handleList(sender, manager)            break;        default: {
    ...
    public void sendResult(CommandSender sender, HideManager.Result result) {
        HideManager manager = plugin.getHideManager();
        switch (result.type()) {        case SUCCESS: {
                        ...
                        break;        }        case DISABLED: send(sender, manager.message("DISABLED", "&cThe hide feature is disabled.")); break;        case DEPENDENCY_MISSING: send(sender, manager.message(
                    "DEPENDENCY-MISSING", "&cHide requires ProtocolLib."))            break;        case NO_PERMISSION: send(sender, manager.message("NO-PERMISSION", "&cYou do not have permission.")); break;        case IN_COMBAT: send(sender, manager.message("IN-COMBAT", "&cYou cannot change hide in combat.")); break;        case COOLDOWN: send(sender, manager.message(
                    "COOLDOWN", "&cWait &f{seconds}s &cbefore changing hide again.",
                    "{seconds}", String.valueOf(result.remainingSeconds())))            break;        case INVALID_ALIAS: send(sender, manager.message("INVALID-ALIAS", "&cInvalid alias.")); break;        case INVALID_SKIN: send(sender, manager.message("INVALID-SKIN", "&cInvalid skin.")); break;        case ALIAS_IN_USE: send(sender, manager.message("ALIAS-IN-USE", "&cThat alias is already in use.")); break;        case NOT_HIDDEN: send(sender, manager.message("NOT-HIDDEN", "&cThat player is not hidden.")); break;        case DATABASE_ERROR: send(sender, "&cUnable to save hide state."); break;
        }
    }

    private void send(CommandSender sender, String message) {
        if (sender instanceof Player) {
            Player player = (Player) label.equalsIgnoreCase("disguise")) {
            return handleDisguise(sender, args, manager);
        }
