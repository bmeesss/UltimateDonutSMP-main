...
public class InvseeManager {
    ...
    private static final List<Integer> DEFAULT_ARMOR_SLOTS = new java.util.ArrayList<>(java.util.Arrays.asList(0,  1,  2,  3));
    ...
    private void refreshOpenSessions() {
        for (InvseeSession session : new java.util.ArrayList<>(sessionsByViewer.values())) {
    ...
    private void syncAllSessions() {
        for (Map.Entry<UUID, Set<UUID>> entry : new java.util.ArrayList<>(viewersByTarget.entrySet())) {
