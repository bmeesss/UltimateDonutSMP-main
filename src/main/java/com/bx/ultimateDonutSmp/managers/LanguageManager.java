...
public class LanguageManager {
    ...
    private static final List<String> BUNDLED_LOCALES = new java.util.ArrayList<>(java.util.Arrays.asList(
    ...
    private static final Set<String> TEXT_KEY_MARKERS = new java.util.LinkedHashSet<>(java.util.Arrays.asList(
    ...
    private static final Set<String> TECHNICAL_KEY_MARKERS = new java.util.LinkedHashSet<>(java.util.Arrays.asList(
    ...
    private static final List<String> MENU_EXCLUDED_PATHS = new java.util.ArrayList<>(java.util.Arrays.asList(
    ...
    public List<String> getAvailableLocales() {
        return new java.util.ArrayList<>(languages.keySet());
    }
    ...
    public List<String> list(String path, List<String> legacyFallback, String... placeholders) {
        ...
        if (value == null) {
            value = legacyFallback == null ? java.util.Collections.emptyList() : legacyFallback;
        }
    ...
    private void mergePlayerLegacyCustomizations(YamlConfiguration target, String locale, boolean force) {
        ...
        mergeFeature(target, "BILLFORD", config.getBillford(), new java.util.ArrayList<>(java.util.Arrays.asList("BILLFORD",  "ACCESS.NPC.DISPLAY_NAME")), force);
        mergeFeature(target, "RTP", config.getRtp(), new java.util.ArrayList<>(java.util.Arrays.asList("DENIED-WORLDS",  "WORLD-SETTINGS")), force);
        mergeFeature(target, "AMETHYST_TOOLS", config.getAmethystTools(), java.util.Collections.emptyList(), force);
        mergeFeature(target, "ENDER_CHEST", config.getEnderChest(), java.util.Collections.emptyList(), force);
        mergeFeature(target, "INVSEE", config.getInvsee(), java.util.Collections.emptyList(), force);
        mergeFeature(target, "FREEZE", config.getFreeze(), new java.util.ArrayList<>(java.util.Arrays.asList("FREEZE.SERVER-NAME",  "FREEZE.ALLOWED-COMMANDS")), force);
        mergeFeature(target, "AUCTION_HOUSE", config.getAuctionHouse(), java.util.Collections.emptyList(), force);
        mergeFeature(target, "ORDERS", config.getOrders(), java.util.Collections.singletonList("CATEGORY_FILTERS"), force);
        mergeFeature(target, "DUELS", config.getDuels(), new java.util.ArrayList<>(java.util.Arrays.asList("ARENA_SETTINGS",  "MAP_SOURCES")), force);
        mergeFeature(target, "CRATES", config.getCrates(), java.util.Collections.singletonList("CRATES"), force);
        mergeFeature(target, "SPAWNERS", config.getSpawners(), java.util.Collections.singletonList("TYPES"), force);
        mergeFeature(target, "SPAWN_STASH", config.getSpawnStash(), java.util.Collections.singletonList("TYPES"), force);
        mergeFeature(target, "NETWORK", config.getNetwork(), new java.util.ArrayList<>(java.util.Arrays.asList(
                "NETWORK-STATUS.SERVERS",  "NETWORK.LOCAL_DISPLAY_NAME",  "NETWORK-STATUS.LOCAL-DISPLAY-NAME")), force);
        mergeFeature(target, "STAFF_MODE", config.getStaffMode(), java.util.Collections.emptyList(), force);
        mergeFeature(target, "SERVER_WIPE", config.getServerWipe(), java.util.Collections.emptyList(), force);
        mergeFeature(target, "WORTH", config.getWorth(), java.util.Collections.singletonList("BLOCK-ITEMS"), force);
        mergeShopText(target, config.getLegacyShop(), force);
        mergeFeature(target, "HIDE", config.getHide(), new java.util.ArrayList<>(java.util.Arrays.asList("ALIASES",  "SKINS",  "STAFF-MARKER")), force);
        mergeFeature(target, "ENCHANTMENTS", config.getEnchantments(), new java.util.ArrayList<>(java.util.Arrays.asList(
    ...
    private static boolean isTranslatablePath(String path, boolean menu) {
        ...
        return menu && new java.util.LinkedHashSet<>(java.util.Arrays.asList("NEXT-BUTTON",  "BACK-BUTTON",  "FIRST-PAGE-BUTTON",  "LAST-PAGE-BUTTON")).contains(leaf);
    }
    ...
    private void rebuildBuiltInTranslations() {
        ...
        for (String path : english.getKeys(true)) {
            ...
            if (source instanceof String sourceText && translated instanceof String) {
            String translatedText = (String) folder.exists() && folder.isDirectory()) {
            File[] files = folder.listFiles();
            if (files != null) {
                for (File file : files) {
                    if (!file.isFile()) {
    ...
    private void rebuildBuiltInTranslations() {
        ...
        for (String path : english.getKeys(true)) {
            ...
            Object source = english.get(path);
            Object translated = localizedValue(path);
            if (source instanceof String sourceText && translated;
                ...
                for (int index = 0; index < size; index++) {
                    ...
                    if (sourceLine instanceof String sourceText && translatedLine instanceof String) {
            String translatedText = (String) source instanceof List<?> sourceList && translated instanceof List<?> translatedList) {
                int size = Math.min(sourceList.size(), translatedList.size());
                for (int index = 0; index < size; index++) {
                    Object sourceLine = sourceList.get(index);
                    Object translatedLine = translatedList.get(index);
                    if (sourceLine instanceof String sourceText && translatedLine;
                        builtInTranslations.putIfAbsent(sourceText, translatedText);
                    }
                }
            }
