...
final class HideProtocolLibBridge implements HidePacketBridge {
    ...
    private PacketContainer createObfuscatedNametagTeam(HideState state) {
        ...
        entries.writeSafely(0, new java.util.ArrayList<>(java.util.Arrays.asList(state.alias(),  state.realNameSnapshot())));
        return packet;
    }
    ...
    private PacketContainer createPlayerInfoRemove(Player target) {
        if (PacketType.Play.Server.PLAYER_INFO_REMOVE.isSupported()) {
            ...
            if (packet.getUUIDLists().size() > 0) {
                packet.getUUIDLists().write(0, java.util.Collections.singletonList(target.getUniqueId()));
        ...
        writePlayerInfoData(packet, java.util.Collections.singletonList(createPlayerInfoData(target, originalProfile(target), null)));
        return packet;
    }
    ...
    private PacketContainer createPlayerInfoAdd(Player viewer, Player target) {
        ...
        writePlayerInfoData(packet, java.util.Collections.singletonList(createPlayerInfoData(target, profile, chatSession)));
        return packet;
    }
...
public final class PlayerInfoListField {
    ...
    public PlayerInfoListField(int index, List<PlayerInfoData> data) {
        this.index = index;
        this.data = data;
    }
    ...
    @Override public boolean equals(Object o) {
        ...
        PlayerInfoListField that = (PlayerInfoListField) o;
        return java.util.Objects.equals(index, that.index) && java.util.Objects.equals(data, that.data);
    }
    @Override public int hashCode() {
        return java.util.Objects.hash(index, data);
    }
}
}
